...
class SaleItem {
  ...
  factory SaleItem.fromJson(Map<String, dynamic> json) {
    return SaleItem(
      ...
      receipt: double.tryParse(json['receipt']?.toString() ?? '0') ?? 0.0,
...
class PurchaseDetail {
  ...
  factory PurchaseDetail.fromJson(Map<String, dynamic> json) {
    return PurchaseDetail(
      ...
      rawQty: double.tryParse(json['raw_qty']?.toString() ?? '0') ?? 0.0,
      unitId: json['unit_id'] ?? 0,
      price: double.tryParse(json['price']?.toString() ?? '0') ?? 0.0,
      subTotal: double.tryParse(json['sub_total']?.toString() ?? '0') ?? 0.0,
      salesQty: double.tryParse(json['sales_qty']?.toString() ?? '0') ?? 0.0,
      returnQty: double.tryParse(json['return_qty']?.toString() ?? '0') ?? 0.0,
      ...
      taxPercentage: double.tryParse(json['tax_percent']?.toString() ?? '0') ?? 0.0,
